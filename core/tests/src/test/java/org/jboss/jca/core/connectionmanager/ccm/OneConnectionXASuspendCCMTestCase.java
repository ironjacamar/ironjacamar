...
/**
 * One connection test case for CCM (XATransaction) using transaction suspend
 *
 * @author <a href="jesper.pedersen@ironjacamar.org">Jesper Pedersen</a>
 */
@RunWith(Arquillian.class)
@Configuration(autoActivate = false)
public class OneConnectionXASuspendCCMTestCase
{
   private static Logger log = Logger.getLogger(OneConnectionXASuspendCCMTestCase.class);
   ...
   public void testOneConnection() throws Throwable
   {
      ...
      ccm.pushMetaAwareObject(new Object(), unsharableResources);
      ...
      log.infof("Before suspend");
      Transaction tx = tm.suspend();

      // New layer
      ccm.pushMetaAwareObject(new Object(), unsharableResources);
      TxLogConnection ci = cf.getConnection();
      ci.clearState();
      assertFalse(ci.isInPool());
      assertEquals("", ci.getState());

      ut.begin();
      assertEquals("3", ci.getState());
      ut.commit();

      ci.close();
      assertEquals("3B8", ci.getState());
      assertTrue(ci.isInPool());

      ccm.popMetaAwareObject(unsharableResources);

      log.infof("After suspend");
      tm.resume(tx);
