...
/**
 * Must provide an implementation of the ManagedConnectionMetaData interface.
 */
public class MCMD implements Rule
{
   /** Section */
   ...
   {
      if (vo != null && Key.MANAGED_CONNECTION_METADATA == vo.getKey())
      {
         if (!(vo.getObject() instanceof ManagedConnectionMetaData))
         {
            List<Failure> failures = new ArrayList<Failure>(1);

            Failure failure = new Failure(Severity.ERROR,
                                          SECTION,
                                          rb.getString("mc.MCMD"),
